...
export function CheckingPage() {
  ...
  function openAddTx() {
    setTxForm({
      ...EMPTY_TX,
      transfer_to_account_id: otherAccounts[0]?.id ?? "",
    });
    setTxErrors({});
  ...
  function validateTx(): boolean {
    ...
    if (txForm.type === "transfer" && !txForm.transfer_to_account_id)
      e.transfer_to_account_id = "Select destination account";
    setTxErrors(e);
  ...
  async function handleSaveTx() {
    ...
    const sourceId = effectiveId === "ALL"
      ? (accounts[0]?.id ?? "")
      : effectiveId;
    ...
    await addTransaction({
      ...
      transfer_to_account_id: txForm.type === "transfer" ? txForm.transfer_to_account_id : undefined,
        ...
        <div className="flex flex-col gap-4">
          ...
          {effectiveId === "ALL" && (
            <Select
              ...
              value={txForm.transfer_to_account_id === "" ? (accounts[0]?.id ?? "") : ""}
          ...
          {txForm.type === "transfer" && (
            <Select
              ...
              placeholder={otherAccounts.length === 0 ? "No other accounts" : undefined}
              value={txForm.transfer_to_account_id}
              onChange={(e) => setTxForm((f) => ({ ...f, transfer_to_account_id: e.target.value }))}
              error={txErrors.transfer_to_account_id}
            />
          )}
