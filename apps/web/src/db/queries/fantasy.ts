...
export async function insertFantasyAccount(
  conn: AsyncDuckDBConnection,
  data: { name: string; platform_type: FantasyPlatformType; starting_balance: number; starting_date: string }
): Promise<string> {
  const id = nanoid();
  await conn.query(`
    INSERT INTO fantasy_accounts (id, name, platform_type, starting_balance, starting_date)
    VALUES ('${id}', '${esc(data.name)}', '${data.platform_type}', ${data.starting_balance}, '${data.starting_date}')
  `);
  return id;
}
...
export async function updateFantasyAccount(
  conn: AsyncDuckDBConnection,
  id: string,
  data: Partial<{ name: string; platform_type: FantasyPlatformType; starting_balance: number; starting_date: string }>
): Promise<void> {
...
export async function getFantasyBalanceSummary(
  ...
  const result = await conn.query(`
    ...
    GROUP BY a.id, a.name, a.platform_type, a.starting_balance, a.starting_date, a.created_at
    ORDER BY a.created_at ASC
  `);

  // Compute current_balance in JS to avoid DECIMAL arithmetic quirks in DuckDB WASM
...
export async function updateFutureStatus(
  ...
  await conn.query(`
    UPDATE fantasy_futures
    SET status = '${status}', settled_date = ${settledDate}
    WHERE id = '${id}'
  `);
}
